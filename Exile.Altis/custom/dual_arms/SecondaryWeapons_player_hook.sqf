...
[] spawn
{
	...
	{
		SecondaryWeaponsAddSecondary = player addAction [format["Put Weapon %1 on back", getText (configFile >> "CfgWeapons" >> (primaryWeapon player) >> "displayName")], { (primaryWeapon player) call SecondaryWeapons_events_addSecondaryWeapon; } , "", 1.6, false, true, "", "(((secondaryWeapon player) =="""") && { (isClass (configFile >> ""CfgWeapons"" >> format[""%1%2"",primaryWeapon player,""_secondary""]))})", 0];
